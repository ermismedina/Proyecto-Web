...
use Dao\Mnt\Peliculas as DaoPeliculas;
use Utilities\Validators;
use Views\Renderer;

class AddPelicula extends PublicController
{
    ...
    public function run(): void
    {
        ...
        $this->processView();
        Renderer::render('mnt/addpelicula', $this->viewData);
    }
    ...
    private function procesarGet()
    {
        if (isset($_GET["mode"])) {
            ...
            if (!isset($this->arrModeDesc[$this->viewData["mode"]])) {
                error_log('Error: (Peliculas) Mode solicitado no existe.');
                \Utilities\Site::redirectToWithMsg(
                    "index.php?page=mnt_peliculas",
        ...
        if ($this->viewData["mode"] !== "INS" && isset($_GET["id"])) {
            ...
            $tmpProducto = DaoPeliculas::getById($this->viewData["ID"]);
    ...
    private function CampoVacio($Campo)
    {
        if (Validators::IsEmpty($this->viewData["$Campo"])) {
            $this->viewData["error_$Campo"][]
                = "El $Campo de la película es requerido";
            $this->hasErrors = true;
        }
    }
            ...
            switch ($this->viewData["mode"]) {
                case 'INS':
                    ...
                    if ($result) {
                        \Utilities\Site::redirectToWithMsg(
                            "index.php?page=mnt_peliculas",
                            "La película se ha Guardado Satisfactoriamente!"
                        );
                    }
                    break;
                case 'UPD':
                    ...
                    if ($result) {
                        \Utilities\Site::redirectToWithMsg(
                            "index.php?page=mnt_peliculas",
                            "La película se ha Actualizado Satisfactoriamente"
                        );
                    }
                    break;
                case 'DEL':
                    $result = DaoPeliculas::delete(
                        intval($this->viewData["ID"])
                    );
                    if ($result) {
                        \Utilities\Site::redirectToWithMsg(
                            "index.php?page=mnt_peliculas",
                            "La película se ha Eliminado Satisfactoriamente"
                        );
                    }
                    break;
            }
